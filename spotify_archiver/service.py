...
def handler(event=None, context=None):
    ...
    # Could be done in two separate calls for a more reusable function, but
...
def get_playlist_ids(session, my_id):
    ...
    raise SpotifyError(
        "Unable to find Discovery Weekly and Discover Weekly Archive "
        "playlists in user's account"
    )
...
def add_tracks_to_playlist(session, user_id, playlist_id, tracks):
    ...
    payload = {"uris": track_uris, "position": 0}
